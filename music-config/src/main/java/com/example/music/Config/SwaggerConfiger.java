...
public class SwaggerConfiger {
    ...
    @Bean
    public Docket createRestApiForSuperAdmin() {
        return new Docket(DocumentationType.SWAGGER_2).enable(true).apiInfo(apiInfo()).select()
                .apis(RequestHandlerSelectors.basePackage("com.example.music.Controller.SuperAdminController"))
                .paths(PathSelectors.any()).build().groupName("超级管理员操作");
    }
